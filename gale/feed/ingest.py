...
class IngestServer:
    def __init__(self, args):
        ...
        self.consumer = GaleKafkaConsumer(
            broker_url=args.broker, group_id=args.group, topics=topics
        )
    ...
    async def start(self):
        ...
        if self.args.mode == "history":
            ...
            start_offset, end_offset = get_history_range(
                self.args.date, self.args.session
            )
            logger.info(
                f"🕰 Time Machine Mode: Replaying {self.args.date} ({self.args.session})"
            )
            ...
            logger.info(
                f"Session Check: Status={session_status}, StartOffset={start_offset}"
            )
        ...
        MAX_TICK_WAIT_MS = (
            50  # Max wait time for a tick before forcing write (Lag Safety)
        )
        ...
        try:
            # [Optimization] Increase Batch Size to 2000 (default 500) to reduce loop overhead
            async for batch_msgs in self.consumer.consume_stream(
                batch_size=2000, running_check=lambda: self.running
            ):
                if not self.running:
                    break
                        ...
                        if topic == "txf-bidask":
                            ...
                            if end_ts_ms and q.timestamp_ms > end_ts_ms:
                                ...
                                if tick_ingestion_complete:
                                    logger.info(
                                        f"🛑 Quote stream reached session end ({q.timestamp_ms}). Drain complete."
                                    )
                            ...
                            if end_ts_ms and t.timestamp_ms > end_ts_ms:
                                ...
                                if time_remaining < 1800000:
                                    logger.info(
                                        f"🛑 Reached session end time via Tick: {end_offset}. (StreamTS: {current_stream_time})"
                                    )
                                    logger.info(
                                        "🌊 Entering Drain Mode: Waiting for quotes to flush pending ticks..."
                                    )
                                    ...
                                    if processed_count % 1000 == 0:
                                        logger.warning(
                                            f"⚠️ Ignored Outlier Tick at {t.timestamp_ms} (Stream: {current_stream_time})."
                                        )
                                    continue
                ...
                # Commit ready ticks
                if ready_ticks:
                    self.ring_buffer.write_batch(
                        ready_ticks, lob_data=ready_lob_metrics
                    )
                ...
                if processed_count > 0 and (
                    processed_count % log_interval < len(ready_ticks)
                ):
                    ...
                    logger.info(
                        f"Processed Ticks: {processed_count}, Quotes: {quote_count}, Pending: {len(pending_ticks)}. LOB Lag: {avg_lag:.1f}ms. Watermark: {max_quote_ts}"
                    )

                # [Optimization] Yield to event loop to reduce CPU usage
                # On Windows, sleep(0.001) often becomes ~15ms.
                # sleep(0) yields execution, but doing it every loop adds overhead.
                # We yield only periodically (e.g., every 1000 batches) or if buffer is full.
                if processed_count % 1000 == 0:
            ...
            if pending_ticks:
                logger.info(
                    f"🧹 Final Flush: Writing {len(pending_ticks)} pending ticks..."
                )
                ready_ticks = []
                ready_lob_metrics = []
                ...
                processed_count += len(ready_ticks)
                logger.info(
                    f"✅ Final Flush Written. Total Processed: {processed_count}"
                )
